...
QuantConnect already covers research, backtesting, optimization, statistical validation, realistic modeling, paper/live workflows and AI-assisted research. TradingView already covers historical/forward strategy testing and paper workflows. MLflow already covers experiment/run tracking, parameters, metrics, artifacts, dataset tracking and reproducibility. Therefore, AI quant research, generic research pipelines, backtesting, optimization, and experiment tracking are not sufficient differentiation by themselves.
...
- Experiment-level LearningRecord.
- Durable `Knowledge` as the canonical semantic memory object.
- An architectural invariant that LearningRecord must not bypass validated evidence when promoted into durable Knowledge.
...
7. What evidence is still missing before the claim can be promoted to durable Knowledge?

The unit of product value is not a run. It is the **claim and its accumulated evidence history**.

## Required product primitive

Introduce a user-facing `Research Claim`:
...
The system preserves positive, negative, failed and inconclusive evidence.
...
3. Map existing Experiment / Finding / Knowledge implementations to those concepts.
...
SaaS expansion proceeds only after a reproducible test demonstrates that a user can answer:

> **"What do I already know about this claim, why do I believe it, what contradicts it, and what evidence is still missing?"**

If the system cannot answer that from stored lineage, the differentiation is not implemented.
