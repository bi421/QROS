"""Strict real-market-data alignment helpers for ResearchOS evidence workflows.

This module contains the former root-script loader logic without exposing a
repository-root executable module. It is importable from the installed
ResearchOS package and fails closed on missing, ambiguous, duplicate, or
...
def _parse_utc_dates(values: pd.Series, path: Path) -> pd.Series:
    ...
    if numeric_fraction == 1.0:
        ...
        if magnitude >= 1e11:
            return pd.to_datetime(
                numeric, unit="ms", utc=True, errors="raise"
            ).dt.normalize()
        if magnitude >= 1e9:
            return pd.to_datetime(
                numeric, unit="s", utc=True, errors="raise"
            ).dt.normalize()
    ...
    return pd.to_datetime(
        values, format="mixed", utc=True, errors="raise"
    ).dt.normalize()
...
def load_and_merge_real_data(
    ...
    for name in ("dxy", "us10y", "vix"):
        merged = merged.merge(
            _normalise_daily_scalar(macro_paths[name], name),
            on="date",
            how="inner",
        )
