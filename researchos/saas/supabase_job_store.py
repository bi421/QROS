...
class SupabaseResearchJobStore(ResearchJobStore):
    ...
    def create(self, job: ResearchJob) -> ResearchJob:
        if job.created_by is None: raise ValueError("Supabase research jobs require created_by")
        ...
        rows=result.data or []
        if len(rows)!=1: raise RuntimeError("Supabase research job insert returned no unique row")
        return self._row_to_job(rows[0])
    def get(self, workspace_id: UUID, job_id: UUID) -> ResearchJob | None:
        result=self._client.table("research_run").select("id,workspace_id,dataset_version_id,workflow_id,status,created_by").eq("workspace_id",str(workspace_id)).eq("id",str(job_id)).limit(1).execute()
        rows=result.data or []; return self._row_to_job(rows[0]) if rows else None
    ...
    def claim(self, workspace_id: UUID, job_id: UUID, owner: str, lease_seconds: int) -> WorkerLease:
        result=self._client.rpc("claim_research_run",{"p_research_run_id":str(job_id),"p_workspace_id":str(workspace_id),"p_lease_owner":owner,"p_lease_seconds":lease_seconds}).execute()
        rows=result.data or []
        if len(rows)!=1: raise RuntimeError("research job is unavailable for claim")
        row=rows[0]; return WorkerLease(self._row_to_job(row),UUID(str(row["lease_token"])))
    def finish(self, workspace_id: UUID, job_id: UUID, lease_token: UUID, target: ResearchJobStatus, error_code: str | None = None) -> ResearchJob:
        ...
        if result.data is not True: raise RuntimeError("stale or invalid worker lease")
        job=self.get(workspace_id,job_id)
        if job is None: raise RuntimeError("finished research job disappeared")
        return job
    def transition(self, workspace_id: UUID, job_id: UUID, expected: ResearchJobStatus, target: ResearchJobStatus) -> ResearchJob:
        result=self._client.table("research_run").update({"status":target.value}).eq("workspace_id",str(workspace_id)).eq("id",str(job_id)).eq("status",expected.value).select("id,workspace_id,dataset_version_id,workflow_id,status,created_by").execute()
        rows=result.data or []
        if not rows: raise ValueError(f"invalid or missing research job transition: {expected.value} -> {target.value}")
        if len(rows)!=1: raise RuntimeError("Supabase research job transition was not unique")
        return self._row_to_job(rows[0])
