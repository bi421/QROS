...
class ResearchWorker:
    ...
    def run_once(self, workspace_id: UUID, job_id: UUID) -> ResearchResult:
        ...
        try:
            ...
            self._store.finish(workspace_id, job_id, lease.token, ResearchJobStatus.FAILED, error_code="executor_error")
            raise
        target = ResearchJobStatus.SUCCEEDED if result.status == "SUCCEEDED" else ResearchJobStatus.FAILED
