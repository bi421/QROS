...
class SupabaseEntitlementStore:
    ...
    def get(self, tenant_id: UUID, plan: Plan) -> Entitlement:
        result = self._client.table("entitlements").select("tenant_id,plan,max_datasets,max_jobs_per_month,max_storage_mb,max_members").eq("tenant_id", str(tenant_id)).limit(1).execute()
        rows = result.data or []
        if not rows:
            template = DEFAULT_ENTITLEMENTS.get(plan.value, DEFAULT_ENTITLEMENTS["free"])
            return Entitlement(tenant_id, template.plan, template.max_datasets, template.max_jobs_per_month, template.max_storage_mb, template.max_members)
        row = rows[0]
        return Entitlement(UUID(str(row["tenant_id"])), str(row["plan"]), int(row["max_datasets"]), int(row["max_jobs_per_month"]), int(row["max_storage_mb"]), int(row["max_members"]))
...
def exceeded(entitlement: Entitlement, usage: UsageSnapshot) -> str | None:
    for field, used in (("max_datasets", usage.datasets), ("max_jobs_per_month", usage.jobs_this_month), ("max_storage_mb", usage.storage_mb), ("max_members", usage.members)):
        limit = entitlement.limit(field)
        if limit > 0 and used >= limit:
            return field
    return None

__all__ = ["DEFAULT_ENTITLEMENTS","Entitlement","EntitlementStore","InMemoryEntitlementStore","SupabaseEntitlementStore","UsageSnapshot","exceeded"]
