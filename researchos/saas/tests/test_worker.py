...
class StubExecutor:
    ...
    def execute(self,job_id):
        ...
        assert self.result is not None; return self.result
def _job(workspace_id): return ResearchJob(id=uuid4(),workspace_id=workspace_id,dataset_version_id=uuid4(),workflow_id="xauusd_m1_frozen_research_v1",status=ResearchJobStatus.QUEUED)
def _result(status="SUCCEEDED"): return ResearchResult(status=status,source_dataset_sha256="0"*64,artifacts=(ResearchArtifact("artifact-1","evidence","1"*64),) if status=="SUCCEEDED" else ())
def test_worker_moves_queued_job_to_succeeded():
    ws=uuid4(); store=InMemoryResearchJobStore(); job=store.create(_job(ws)); ex=StubExecutor(_result()); result=ResearchWorker(store,ex).run_once(ws,job.id)
    assert result.status=="SUCCEEDED"; assert ex.calls==1; assert store.get(ws,job.id).status==ResearchJobStatus.SUCCEEDED
def test_worker_marks_job_failed_when_executor_raises():
    ws=uuid4(); store=InMemoryResearchJobStore(); job=store.create(_job(ws)); ex=StubExecutor(error=RuntimeError("scientific failure"))
...
def test_worker_cannot_run_job_from_another_workspace():
    store=InMemoryResearchJobStore(); owner,other=uuid4(),uuid4(); job=store.create(_job(owner))
...
def test_worker_cannot_double_claim_active_job():
    ws=uuid4(); store=InMemoryResearchJobStore(); job=store.create(_job(ws)); first=store.claim(ws,job.id,"worker-a",900)
...
def test_stale_lease_token_cannot_finish_job():
    ws=uuid4(); store=InMemoryResearchJobStore(); job=store.create(_job(ws)); lease=store.claim(ws,job.id,"worker-a",900)
