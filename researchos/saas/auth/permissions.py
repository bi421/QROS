...
for action in Action:
    _POLICY[WorkspaceRole.OWNER].update((resource, action) for resource in Resource)
    _POLICY[WorkspaceRole.ADMIN].update(
        (resource, action) for resource in Resource if resource != Resource.BILLING
    )
_POLICY[WorkspaceRole.ADMIN].update(
    {(Resource.BILLING, Action.READ), (Resource.BILLING, Action.LIST), (Resource.BILLING, Action.UPDATE)}
)
for resource in (
    Resource.DATASET,
