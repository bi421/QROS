...
The adapter deliberately composes the existing ``ResearchRepository`` generic
object store instead of changing its central object registry. This keeps the
new claim contract additive while the wider evidence graph integration is
...
class ResearchClaimRepository:
    """Persist and retrieve ``ResearchClaim`` objects through the canonical store."""
