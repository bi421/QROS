...
A Research Claim is deliberately narrower than a strategy or report. It states
exactly what is being asserted, under which population and horizon, what would
falsify it, and which evidence is required before the assertion may become
trusted knowledge.

The object supports a two-phase contract:
1. exploratory editing before a research plan is locked;
2. immutable plan locking followed by versioned forks for material changes.
"""
...
class ResearchClaim(BaseObject):
    ...
    def lock_plan(self, plan: ResearchPlan) -> str:
        """Lock the confirmatory research plan exactly once and return its hash."""
    ...
    def add_experiment(self, experiment_id: str) -> None:
        """Attach an experiment without mutating the locked research plan."""
    ...
    def add_evidence(self, evidence_hash: str) -> None:
        """Attach a content-addressed evidence artifact by hash."""
    ...
    def fork_version(self, statement: str | None = None) -> "ResearchClaim":
        """Create a new claim version; the current claim itself remains unchanged."""
    ...
    def clone(self) -> "ResearchClaim":
        """Return a detached copy for safe caller-side inspection."""
