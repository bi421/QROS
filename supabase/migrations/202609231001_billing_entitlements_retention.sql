-- Billing entitlements and tenant lifecycle retention contract.
...
create policy tenant_select on public.entitlements for select to authenticated
using (auth.jwt() ->> 'tenant_id' = tenant_id::text);
create policy tenant_insert on public.entitlements for insert to authenticated
with check (auth.jwt() ->> 'tenant_id' = tenant_id::text);
create policy tenant_update on public.entitlements for update to authenticated
using (auth.jwt() ->> 'tenant_id' = tenant_id::text)
with check (auth.jwt() ->> 'tenant_id' = tenant_id::text);
create policy tenant_delete on public.entitlements for delete to authenticated
using (auth.jwt() ->> 'tenant_id' = tenant_id::text);

-- Every tenant-owned row participates in the retention lifecycle.
...
alter table public.entitlements add column if not exists deleted_at timestamptz;
...
-- Atomic workspace lifecycle operation. It records audit hashes before hiding content.
...
begin
  ...
  insert into public.tenant_deletion_tombstone
    (id, workspace_id, tenant_id, deleted_at, purge_after)
  values
    (receipt, p_workspace_id, p_workspace_id, p_deleted_at, p_deleted_at + interval '30 days')
