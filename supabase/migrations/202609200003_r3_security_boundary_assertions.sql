...
begin
    ...
    where n.nspname = 'public'
      ...
      and c.relname in (
          ...
          'audit_log', 'api_rate_limit', 'billing_event'
      )
      and not c.relrowsecurity;

    if missing_rls is not null then
        raise exception 'R3 security gate: RLS disabled on: %', missing_rls;
    end if;
    ...
    if exposed_grants is not null then
        raise exception 'R3 security gate: client table grants detected: %', exposed_grants;
    end if;
    ...
    if unsafe_definers is not null then
        raise exception 'R3 security gate: SECURITY DEFINER function without pinned search_path: %', unsafe_definers;
    end if;
    ...
    if not exists (
        ...
        raise exception 'R3 security gate: qros-datasets bucket must remain private';
    end if;
    ...
    if not exists (
        ...
        where schemaname = 'public'
          and tablename = 'api_rate_limit'
          and policyname = 'api_rate_limit_no_client_access'
          and qual = 'false'
          and with_check = 'false'
    ) then
        raise exception 'R3 security gate: api_rate_limit explicit deny policy missing';
    end if;
    ...
    if not exists (
        ...
        from pg_policies
        where schemaname = 'public'
          and tablename = 'billing_event'
          and policyname = 'billing_event_no_client_access'
          and qual = 'false'
          and with_check = 'false'
    ) then
        raise exception 'R3 security gate: billing_event explicit deny policy missing';
    end if;
end $$;
