-- Allow a previously flagged reconciliation to close atomically only after
-- the caller has independently verified the resource is absent.
...
begin
    ...
    if not (
        ...
        or (current_operation.state = 'RECONCILIATION_REQUIRED' and p_state = 'COMPLETED')
...
comment on function private.transition_retention_deletion_operation(uuid, text, text, text, text) is
    'Atomic tenant-scoped retention state transition; service-role only. Reconciliation may close only to COMPLETED.';
