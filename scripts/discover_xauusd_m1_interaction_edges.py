        ...
        if field.startswith("preceding_return_") or field == "macd_histogram":
            for op, symbol in ((lambda value: value > 0, ">0"), (lambda value: value < 0, "<0")):
                out.append(
                    Candidate(
                        f"{field}{symbol}",
                        lambda c, field=field, op=op: _finite(c.get(field)) and op(float(c[field])),
                    )
                )
        if field == "rsi":
...
def run(
    ...
    events = [
        event for event in report.get("events_data", [])
    ...
    while start + train_size + validation_size <= len(events):
        ...
        train = [
            row for row in train_pool
            if _time((row.get("outcome") or {}).get("data_availability", {}).get("realized_end_1d", "")) < validation_start
        ]
            ...
            fold.update(
                {
                    ...
                    "brier_improvement": _brier(baseline, selected_valid) - _brier(model, selected_valid),
                }
            )
        else:
            fold.update(
    ...
    result = {
        "stage": "XAUUSD_M1_NESTED_INTERACTION_EDGE_DISCOVERY",
        ...
        "outer_support_failures": sum(not fold["outer_support_met"] for fold in folds),
        "selected_oos_events": sum(fold["selected_validation_events"] for fold in evaluated),
        "positive_folds": positive,
        "negative_folds": negative,
        "positive_fold_rate": positive / len(improvements) if improvements else None,
        "mean_brier_improvement": statistics.fmean(improvements) if improvements else None,
        "folds": folds,
        "scientific_gate": {
            "status": "NO_EDGE_OR_INCONCLUSIVE",
            "reason": "Interaction discovery is exploratory; confirmation must use untouched data and the frozen B-level statistical gate.",
        },
    }
