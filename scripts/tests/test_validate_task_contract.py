...
def test_missing_required_section_fails(tmp_path: Path) -> None:
    ...
    assert "missing required section: ## Risk" in errors
...
def test_unfilled_required_field_fails(tmp_path: Path) -> None:
    ...
    assert any("unfilled required field: - Goal:" == error for error in errors)


def test_empty_allowed_scope_fails(tmp_path: Path) -> None:
    path = tmp_path / "task.md"
    path.write_text(
        VALID.replace("- scripts/validate_task_contract.py", ""),
        encoding="utf-8",
    )

    errors = validate_contract(path)

    assert "scope allowed list must contain at least one non-empty item" in errors


def test_missing_acceptance_checklist_fails(tmp_path: Path) -> None:
    path = tmp_path / "task.md"
    path.write_text(
        VALID.replace("- [ ] Validator rejects incomplete contracts", ""),
        encoding="utf-8",
    )

    errors = validate_contract(path)

    assert "acceptance criteria must contain at least one non-empty checklist item" in errors
